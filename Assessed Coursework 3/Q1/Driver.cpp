...
This driver is intended to solve a linear ODE using the Forward Euler method
...
void main()
{
	...
	ForwardEulerSolver* pQuestion1di = new ForwardEulerSolver(*pODE, pU, InitialTime, FinalTime, h, "output.dat",1 ,25);
	...
	for (h = 0.0025; h<1.001; h = h + 0.0025)
	{
		...
		ForwardEulerSolver* pQuestion1dii = new ForwardEulerSolver(*pODE, pU, InitialTime, FinalTime, h);
		//Call the numerical error method from the solver
		ApproximationError = pQuestion1dii->NumericalError();
